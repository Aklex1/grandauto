...
# На основе чего строим контент-план. От этого зависит, что модель подбирает:
# книги, темы, разбор своих материалов или тренды ниши.
CONTENT_SOURCES: dict[str, str] = {
    "books": "Саммари книг — по одной нон-фикшн книге на ролик",
    "topics": "Темы ниши — без привязки к книгам, свои разборы",
    "cases": "Разборы ситуаций — типичные истории аудитории",
    "trends": "Тренды и вопросы — то, что аудитория спрашивает прямо сейчас",
}

_PLAN_FORMATS = {
    "books": ("Формат каждого ролика: саммари одной нон-фикшн книги — главная идея, "
              "5-8 ключевых мыслей, практические правила и упражнения."),
    "topics": ("Формат каждого ролика: разбор одной темы канала — в чём проблема, "
               "почему так происходит, что делать. Без опоры на книги."),
    "cases": ("Формат каждого ролика: разбор типичной ситуации из жизни аудитории — "
              "что произошло, где ошибка, как надо было."),
    "trends": ("Формат каждого ролика: ответ на живой вопрос аудитории или разбор того, "
               "что обсуждают в нише прямо сейчас."),
}

_PLAN_PICK = {
    "books": ("Подбери {count} книг. Требования к подборке:\n"
    ...
    "topics": ("Подбери {count} тем. Требования:\n"
               "- каждая тема — отдельная законченная мысль, а не кусок другой;\n"
               "- конкретика вместо общих слов вроде «саморазвитие»;"),
    "cases": ("Подбери {count} ситуаций. Требования:\n"
              "- узнаваемые истории, в которых зритель увидит себя;\n"
              "- у каждой понятная развилка: где именно всё пошло не так;"),
    "trends": ("Подбери {count} вопросов и тем. Требования:\n"
               "- то, что действительно спрашивают, а не выдуманные поводы;\n"
               "- формулировки словами аудитории, а не редактора;"),
}


def content_plan(channel_name: str, topic: str, count: int, audience: str,
                 existing: list[str] | None = None, source: str = "books",
                 style_hint: str = "", description: str = "") -> list[dict]:
    """Промпт контент-плана. source задаёт, из чего плана вообще состоит."""
    source = source if source in CONTENT_SOURCES else "books"
    exclude = ""
    if existing:
        exclude = "\nНЕ повторяй уже запланированное:\n- " + "\n- ".join(existing[:200])
    about = f"\nО канале: {description.strip()}" if description.strip() else ""
    refs = f"\nОриентиры по стилю, взятые из образцов канала: {style_hint}" \
        if style_hint.strip() else ""
    # У формата «книги» поле book_title — название книги, у остальных источников
    # книг нет, и туда кладётся тема. Схема ответа общая, чтобы не плодить ветки
    # разбора: конвейер дальше работает с этим полем одинаково.
    label = "название книги на русском" if source == "books" else \
        "короткое название темы ролика (2-5 слов)"
    author = "автор" if source == "books" else "оставь пустым"

    user = f"""Составь контент-план для YouTube-канала «{channel_name}».

Тематика канала: {topic}{about}
Аудитория: {audience}
{_PLAN_FORMATS[source]} Длительность 7-10 минут.{refs}

{_PLAN_PICK[source].format(count=count)}
...
[{{"book_title":"{label}",
  ...
  "angle":"1-2 предложения: под каким углом подаём и кому это нужно",

...
Уведомления ЮMoney приходят сюда же, на сервер. Порядок такой:

1. уведомление без метки пропускается — так ЮMoney проверяет адрес при сохранении;
2. всё остальное сначала уходит на сайт: он находит пользователя по метке
   платежа и пополняет ему баланс — неважно, зашёл человек по почте, через
   ВК или из Telegram;
3. если сайт отвечает, что такого платежа не знает, уведомление отдаётся
   боту по адресу `BOT_WEBHOOK` — на случай платежей, заведённых не сайтом.
   Не нужен — оставьте переменную пустой.
...
Журнал платежей: `GET /payments/log?limit=50` с заголовком `X-API-Key`.
Ответ сайта на каждое уведомление содержит поле `credited` — по нему и
...
это значит, что поддельное уведомление тоже будет принято.
...
| `BOT_WEBHOOK` | куда отдавать платежи, которых сайт не знает | `http://127.0.0.1:8000/yoomoney-webhook` |
